...
Параметры **w_offs** и **h_offs** были введены и используются для работы с нестандартными/"кривыми"/бракованными и т.п. дисплеями, у которых начало координат физической матрицы дисплея и начало координат поля контроллера дисплея не совпадают, т.е. смещены друг относительно друга. Для "правильного" дисплея эти параметры должны быть равны 0. Для "неправильного" определяются исходя из конкретного образца, имеющегося на руках. w_offs определяет смещение по горизонтали матрицы дисплея в поле контроллера дисплея, а h_offs - аналогичное смещение, но по вертикали. 
