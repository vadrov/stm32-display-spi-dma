...
void LCD_ReadImage(LCD_Handler* lcd, uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t *data)
{
	...
	while (len--) {
		...
		//Если дисплейный модуль с тачскрином, а драйвер тачскрина не подключен, то подтяните вывод тачскрина
		//T_CS к питанию (он не должен болтаться в "воздухе").
		//Что интересно, первый признак превышения скорости - это обмен местами составляющих цвета b и r.
		spi->DR = 0x00; //NOP
		while (!(spi->SR & SPI_SR_RXNE)) { __NOP(); }//Ожидаем прием ответа от контроллера дисплея
		b = spi->DR;
		spi->DR = 0x00; //NOP
