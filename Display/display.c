...
{
	//Проверяем допустимость координат окна
	if (x >= lcd->Width || y >= lcd->Height) return;
	//Корректируем ширину и высоту окна при выходе его за пределы границ дисплея
	if (x + w - 1 >= lcd->Width) w = lcd->Width - x;
	if (y + h - 1 >= lcd->Height) h = lcd->Height - y;
	//Объявляем и настраиваем переменные сдвига окна.
	//Процедура универсальная, поэтому введем указатель coord на переменную с координатой,
	//по которой двигаемся в ходе сдвига. Также вводим шаги по координатным осям: sign_x, sign_y.
	//Они будут реализовывать приращение координат при различных направлениях сдвига. Переменная
	//sign определяет шаг по координате, на которую указывает coord, и используется для
	//расчета соответствующей координаты после операции по копированию и переносу строки/столбца.
	//Переменная sizе определяет размер буфера для чтения пиксельных данных. Она определяется высотой
	//либо шириной окна в зависимости от типа сдвига. shift_cycles определяет количество строк/столбцов
	//в окне, которые подлежат копированию/переносу. При этом количество копирований/переносов (сдвигов)
	//равно shift_cycles - 1. Копирование в буфер содержимого предыдущих строк/столбцов осуществляется
	//процедурой LCD_ReadImage. Вывод содержимого из буфера в текущие строки/столбцы осуществляется
	//процедурой LCD_DrawImage
	uint16_t size, w_scroll, h_scroll, *coord, shift_cycles;
	int sign, sign_x, sign_y;
	//Сдвиг влево либо вправо
	if (scroll_type == LCD_SCROLL_LEFT || scroll_type == LCD_SCROLL_RIGHT) {
		size = h;         //Размер буфера равен высоте окна
		shift_cycles = w; //Число строк для сдвига равно ширине окна
		w_scroll = 1;	  //Ширина считываемого/выводимого блока данных 1 пиксель
		h_scroll = h;     //Высота считываемого/выводимого блока данных h пикселей
		sign_y = 0;		  //Шаг по координате y равен 0
		if (scroll_type == LCD_SCROLL_LEFT) { //Для левого сдвига:
			sign_x = 1; //шаг по координате x равен 1
		}
		else {  //Для правого сдвига:
			x += w - 1;  //начальная координата х определяется, как x = x + w - 1
			sign_x = -1; //шаг по координате x равен минус 1
		}
		coord = &x;      //Указатель координаты, по которой шагаем указывает на координату x
		sign = sign_x;   //Шаг перемещения определяет шаг по координате x
	}
	else {
		size = w;		 //Размер буфера равен ширине окна
		shift_cycles = h;//Число строк для сдвига равно высоте окна
		w_scroll = w;    //Ширина считываемого/выводимого блока данных w пикселей
		h_scroll = 1;	 //Высота считываемого/выводимого блока данных 1 пиксель
		sign_x = 0;		 //Шаг по координате x равен 0
		if (scroll_type == LCD_SCROLL_UP) { //Для сдвига вверх:
			sign_y = 1;	 //шаг по координате y равен 1
		}
		else {   //Для сдвига вниз:
			y += h - 1;  //начальная координата y определяется, как y = y + h - 1
			sign_y = -1; //шаг по координате y равен минус 1
		}
		coord = &y;		//Указатель координаты, по которой шагаем указывает на координату y
		sign = sign_y;  //Шаг перемещения определяет перемещение по координате y
	}
	//Ширина либо высота окна (в зависимости от вида скроллинга) должна быть не меньше 2 пикселей
	if (shift_cycles < 2) return;
	//Массивы для пиксельных данных, читаемых из памяти контроллера дисплея
	uint16_t pix_cycl[size]; //Для сохранения первой строки/столбца при использовании циклического сдвига.
	uint16_t pix_data[size]; //Для временного хранения данных при поэтопном сдвиге остальных строк.
	if (cyclic) { //Если опеределен циклический перенос, то читаем в буфер первую строку/столбец
		LCD_ReadImage(lcd, x, y, w_scroll, h_scroll, pix_cycl);
	}
	//Цикл по количеству сдвигов (количество строк/столбцов за минусом 1)
	for (int i = 0; i < shift_cycles - 1; i++) {
		//Читаем в буфер следующую строку/столбец
		LCD_ReadImage(lcd, x + sign_x, y + sign_y, w_scroll, h_scroll, pix_data);
		//Выводим на место текущей строки/столбца считанные данные из следующей
		LCD_DrawImage(lcd, x, y, w_scroll, h_scroll, pix_data, 1);
		//Переходим к следующей строке/столбцу
		*coord += sign;
	}
	if (cyclic) { //Если опеределен циклический перенос, то считанные данные первой строки/столбца
		//выводим на место последней строки/столбца
		LCD_DrawImage(lcd, x, y, w_scroll, h_scroll, pix_cycl, 1);
	}
}
