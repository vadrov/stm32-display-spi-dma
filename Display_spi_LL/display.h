...
#ifndef LCD_DYNAMIC_MEM					//Размер статического буфера драйвера дисплея.
#define LCD_STATIC_WORK_BUFFER_SIZE	750	//Используется при статическом выделении памяти.
										//Размер этого буфера должен быть равен удвоенному размеру
										//максимального блока данных, пересылаемых на дисплей.
										//Размер этого блока выбирается из ширины строки дисплея (в пикселях) или
										//количества пикселей в букве применяемого шрифта максимального размера,
										//в зависимости от того, что больше.
										//Например, если размер строки дисплея 320 пикселей, а максимальный размер
										//шрифта 15х25 = 375 пикселей, то выбираем 375 и умножаем на 2 (удвоенный буфер).
										//Таким образом, размер статического буфера для этого примера задаем равным 750.
#endif
/* --------------------------------------------------------------------------------------------------------------------- */
